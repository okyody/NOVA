...
async function loadPlatformCatalog() {
  try {
    ...
    renderSimpleRows('platform-catalog', lastPlatformCatalog.map((item) =>
      `${item.platform} | required=${(item.required_fields || []).join(',')} | modes=${(item.modes || []).join(',')} | events=${(item.event_types || []).join(',')}`
    ));
...
async function loadPlatformTemplates() {
  try {
    ...
    renderSimpleRows('platform-catalog', items.map((item) => `${item.platform} template ready`));
    document.getElementById('platform-config-editor').value = items.map((item) => JSON.stringify(item.template)).join('\n');
    wizardLog('Platform templates loaded.');
  } catch (err) {
...
async function loadPlatformConfig() {
  try {
    const result = await getJson('/api/platforms/config');
    const items = result.items || [];
...
async function validatePlatformConfig() {
  try {
    ...
    renderSimpleRows('platform-status', (result.items || []).map((item) => `${item.platform} | valid=${item.valid} | reason=${item.reason || 'ok'}`));
...
async function savePlatformConfig() {
  try {
    ...
    document.getElementById('platform-config-status').textContent = `Saved ${result.count} platform config item(s). Restart required=${result.restart_required}`;
...
async function loadPlatformStatus() {
  try {
    ...
    renderSimpleRows('platform-status', configured.map((item) => {
      const state = runtime[item.platform] || {};
      const valid = validation.find((entry) => entry.platform === item.platform);
      return `${item.platform} | mode=${item.mode || 'n/a'} | room=${item.room_id || item.channel || item.live_chat_id || 'n/a'} | health=${state.health || 'n/a'} | running=${state.running ?? false} | valid=${valid?.valid ?? true}`;
    }));
...
async function sendPlatformTestEvent() {
  try {
    ...
    document.getElementById('platform-debug-status').textContent = `Test event sent: ${result.source} / ${result.event_type} / ${result.event_id}`;
